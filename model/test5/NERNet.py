...
class NERNet(nn.Module):
    def __init__(self,  hparams,
        ...
        # 1b. matrix positional embedding

        self.pos_embedding = nn.Parameter(torch.randn(1, hparams['window_size'], embedding_word_dim) * 0.5)
        ...
        #     nn.Conv1d(embedding_word_dim, embedding_word_dim, 3, padding=1),
        #     nn.GroupNorm(1,embedding_word_dim),
        #     nn.ReLU(),

        #     nn.Conv1d(embedding_word_dim, lstm_word_output_dim, 3, padding=1),
        #     nn.GroupNorm(1,lstm_word_output_dim),
        #     nn.ReLU(),
        # )

        # 3. classifier (and optionally crf)

        # self.fc1 = nn.Linear(lstm_word_output_dim, lstm_word_output_dim // 2)
        # self.ln1 = nn.LayerNorm(lstm_word_output_dim // 2)
        # self.fc2 = nn.Linear(lstm_word_output_dim // 2, lstm_word_output_dim // 2)
        # self.ln2 = nn.LayerNorm(lstm_word_output_dim // 2)

        self.classifier = nn.Linear(lstm_word_output_dim, self.n_labels)

        # self.cnn2 = nn.Conv1d(self.n_labels, self.n_labels, 3, padding=1)
    ...
    def compute_outputs(self, x_word, x_char):
        ...
        # if x_word.shape[1] == self.pos_embedding.shape[1]:
        #     x_word = x_word + self.pos_embedding
        # elif x_word.shape[1] < self.pos_embedding.shape[1]:
        #     x_word = x_word + self.pos_embedding[:,:x_word.shape[1]]
        # else:
        #     x_word[:,:self.pos_embedding.shape[1]] = x_word[:,:self.pos_embedding.shape[1]] + self.pos_embedding

        x_word = self.lstm_main(x_word)[0] # + self.cnn1(x_word.permute(0,2,1)).permute(0,2,1)

        # x_word = self.fc1(x_word)
        # x_word = self.ln1(x_word)
        # x_word = self.relu(x_word)

        # x_word = self.fc2(x_word)
        # x_word = self.ln2(x_word)
        # x_word = self.relu(x_word)

        x_word = self.classifier(x_word) # (batch, sentence_len, n_lables)
        # x_word = self.cnn2(x_word.permute(0,2,1)).permute(0,2,1)
    ...
    def compute_loss(self, x, y_true, mask = None):
        ...
        loss = self.crf.forward(x,y_true,mask)

        loss = -(torch.sum(loss) / mask.float().sum() )
