...
class NERDataset(Dataset):
    def __init__(self,  flile_path:str, vocabulary = None, vocabulary_pos = None, 
                        pos_tagger = None, params = None):
        """
        Args:
            - file_path: path to the file to open
            - vocabulary: the vocabulary dictionary obtained by gensim, composed of 'key_to_index' and 'index_to_key' for the words
            - vocabulary_pos: the vocabulary dictionary for POS, composed of 'key_to_index' and 'index_to_key' for the words. If not None, then POS tagging is added
            - pos_tagger: POS tagger by spacy
            - params: the dictionary needed to initialize this class (see notebook nlp-hw1_test2 for more details)
        """
        self.vocabulary = vocabulary
        self.vocabulary_pos = vocabulary_pos
    ...
    @staticmethod
    def generate_windows(data, window_size, window_shift, pad_token = None, pad_index = -1, vocabulary_pos = None, pos_tagger = None):
        """
        Args:
            ...
            - vocabulary_pos: the vocabulary for pos tagging
            - pos_tagger: the pos tagger by spacy \n
        Returns:
            list of dictionaries, each dictionary has: \n
        ...
        for sentence in data:
            ...
            sentence_pos = spacy.tokens.Doc(pos_tagger.vocab, words=sentence['inputs'])
            sentence_pos = pos_tagger( sentence_pos )
            sentence_pos = [token.pos_ for token in sentence_pos]
            windowed_pos = [sentence_pos[i:i+window_size] for i in range(0, len(sentence_pos), window_shift)]

            try:
                assert len(sentence_pos) == len(sentence['inputs'])
            except:
                print(sentence['inputs'], sentence_pos)
                raise RuntimeError('Sentence POS and input not equal')
            
            for window_input, window_output, window_pos in zip(windowed_inputs, windowed_outputs, windowed_pos):
                ...
                window_pos = window_pos + [pad_token]*(window_size - len(window_pos))

                windowed_data.append({ 'inputs':window_input , 'outputs':window_output , 'pos':window_pos })

        return windowed_data

    @staticmethod
    def generate_windows_sentence(data, window_size, window_shift, pad_token = None, pad_index = -1, vocabulary_pos = None, pos_tagger = None):
        """
        Args:
        ...
        for sentence in data:
            ...
            sentence_pos = spacy.tokens.Doc(pos_tagger.vocab, words=sentence)
            sentence_pos = pos_tagger( sentence_pos )
            sentence_pos = [token.pos_ for token in sentence_pos]
            windowed_pos = [sentence_pos[i:i+window_size] for i in range(0, len(sentence_pos), window_shift)]

            for window_input, window_pos in zip(windowed_inputs, windowed_pos):
                window_input = window_input + [pad_token]*(window_size - len(window_input))
                window_pos = window_pos + [pad_token]*(window_size - len(window_pos))

                windowed_data.append({ 'inputs':window_input , 'pos':window_pos })

        return windowed_data

    @staticmethod
    def prepare_batch_fn(vocabulary, label2id, unk_tok, vocabulary_pos):
        """
        Args:
            - vocabulary: the vocabulary dictionary obtained by gensim, composed of 'key_to_index' and 'index_to_key' for the words
            - label2id: the dictionary that converts a label to its id
            - unk_token: the string representation of the unknown token
            - vocabulary_pos: the vocabulary POS dictionary composed of 'key_to_index' and 'index_to_key' \n
        Returns:
        ...
        def prepare_batch(batch: List[Dict]) -> List[Dict]:
            ...
            pos = [NERDataset.encode_sentence_pos(sample, vocabulary_pos) for sample in pos]
        
            return { 'inputs': as_tensor(inputs) , 'outputs': as_tensor(outputs) , 'pos': as_tensor(pos) }
    ...
    @staticmethod
    def encode_sentence_pos(sentence, pos_vocabulary):
        """
        Args:
            - sentence: a list of strings (POS)
            - pos_vocabulary: the vocabulary POS dictionary composed of 'key_to_index' and 'index_to_key' \n
        Returns:
            the encoded version of the POS sentence
        """
        encoded = [pos_vocabulary['key_to_index'][pos_word] for pos_word in sentence]
        return encoded
    ...
    def save_vocabulary_pos(self, path):
        np.save(path, self.vocabulary_pos)
