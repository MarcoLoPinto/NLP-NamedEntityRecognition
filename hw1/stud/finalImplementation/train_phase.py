...
def train_and_evaluate(
    ...
    Args:
        ...
        - save_best: if True, then the best model that surpasses min_f1_score will be saved
        - save_path_name: path and name for the best model to be saved 
        - min_f1_score: minimum score acceptable in order to be saved 
    ...
    for epoch in range(epochs):
        ...
        if valid_data_output is not None and valid_dataloader is not None:
            ...
            if save_best and save_path_name is not None:
                if valid_f1_history[-1] == max(valid_f1_history) and valid_f1_history[-1] >= min_f1_score:
                    print(f'----- Best value obtained, saving model -----')
                    final_model.model.save_weights(save_path_name)
            if valid_f1_history[-1]  < 0.0001 and epoch > 3:
                print(f'----- f1 not increasing, break -----')
                break
