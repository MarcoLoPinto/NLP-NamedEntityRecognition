...
class NERNet(nn.Module):
    def __init__(self,  hparams,
        ...
        Args:
            - hparams: hyperparams to use, such as:
                ...
                - lstm_parameters: a dictionary to create word LSTM(s) with parameters:
                ...
                - char_lstm_parameters: a dictionary to create char LSTM(s) with parameters:
                    - hidden_size: the hidden size of the LSTM
                    - bidirectional: if the LSTM is bidirectional or not
                    - num_layers: number of layers of the LSTM
                    - dropout: dropout value, between 0 and 1 (if num_layers > 1)
        ...
        self.lnorm_char_emb = nn.LayerNorm(embedding_char_dim)

        # 2a. char LSTM

        self.lstm_char = nn.LSTM(
            input_size = embedding_char_dim, 
            hidden_size = hparams['char_lstm_parameters']['hidden_size'], 

            bidirectional = hparams['char_lstm_parameters']['bidirectional'], 
            num_layers = hparams['char_lstm_parameters']['num_layers'], 
            dropout = hparams['char_lstm_parameters']['dropout'] if hparams['char_lstm_parameters']['num_layers'] > 1 else 0,
            batch_first = batch_first  
        )

        lstm_char_output_dim = 2*hparams['char_lstm_parameters']['hidden_size'] if hparams['char_lstm_parameters']['bidirectional'] is True else hparams['char_lstm_parameters']['hidden_size']
        ...
        lstm_main_output_dim = 2*hparams['lstm_parameters']['hidden_size'] if hparams['lstm_parameters']['bidirectional'] is True else hparams['lstm_parameters']['hidden_size']

        # 3. classifier (and optionally crf)

        self.fc1 = nn.Linear(lstm_main_output_dim, lstm_main_output_dim // 2)
        self.ln1 = nn.LayerNorm(lstm_main_output_dim // 2)

        self.classifier = nn.Linear(lstm_main_output_dim // 2, self.n_labels)
    ...
    def compute_outputs(self, x_word, x_char):
        ...
        x_char = self.lnorm_char_emb(x_char)
